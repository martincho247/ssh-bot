...
# ================================================
# CREAR BOT CON PLANES SEPARADOS
# ================================================
echo -e "\n${CYAN}${BOLD}🤖 CREANDO BOT CON PLANES SEPARADOS...${NC}"
...
# Crear bot.js CON PLANES SEPARADOS
echo -e "${YELLOW}📝 Creando bot.js con planes separados...${NC}"
...
// ✅ FLUJO PRINCIPAL CON PLANES SEPARADOS
    ...
    // SELECCIÓN DE PLAN DIARIO PARA RENOVACIÓN
        ...
        if (text === '1' || text === '2') {
            ...
            if (planData) {
                ...
                await setUserState(phone, 'renovation_discount', { 
                    username: username,
                    plan: `${planData.days}d`,
                    days: planData.days,
                ...
                await client.sendMessage(phone, `🔄 *RENOVACIÓN: ${username} - ${planData.days} DÍAS*

**¿Tienes un cupón de descuento para la renovación?**
Responde: sí o no.`, { sendSeen: false });
            }
        }
        else if (text === '0') {
...
Escribe *menu* para ver las opciones.`, { sendSeen: false });
    ...
    // PREGUNTA POR CUPÓN EN RENOVACIÓN
    else if (userState.state === 'renovation_discount') {
        const stateData = userState.data || {};
        
        if (text === 'sí' || text === 'si' || text === 'sí.' || text === 'si.') {
            await setUserState(phone, 'renovation_entering_discount', stateData);
            await client.sendMessage(phone, '📝 Por favor, escribe tu código de descuento para la renovación:', { sendSeen: false });
        }
        else if (text === 'no' || text === 'no.' || text === 'no gracias') {
            await processRenovation(phone, stateData, null);
        }
        else {
            await client.sendMessage(phone, 'Por favor responde: *sí* o *no*', { sendSeen: false });
        }
    }
    // INGRESAR CÓDIGO DE DESCUENTO PARA RENOVACIÓN
    else if (userState.state === 'renovation_entering_discount') {
        const stateData = userState.data || {};
        const discountCode = text.trim();
        
        await processRenovation(phone, stateData, discountCode);
    }
...
// ✅ FUNCIÓN PARA PROCESAR PAGO DE COMPRA
...
async function processRenovation(phone, renovationData, discountCode) {
    config = loadConfig();
...
✅ Después del pago, tu cuenta será renovada automáticamente.`;
            ...
            if (fs.existsSync(payment.qrPath)) {
                try {
                    const media = MessageMedia.fromFilePath(payment.qrPath);
                    await client.sendMessage(phone, media, { 
                        caption: `📱 *Renovación - Escanea con MercadoPago*\n\n${renovationData.username} - ${renovationData.days} días - ${amountText}`, 
                        sendSeen: false 
                    });
                } catch (qrError) {
                    console.error(chalk.red('⚠️ Error enviando QR:'), qrError.message);
...
console.log(chalk.green('\n🚀 Inicializando bot con planes separados...\n'));
client.initialize();
BOTEOF

echo -e "${GREEN}✅ Bot creado con planes separados${NC}"
...
show_header() {
    clear
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                🎛️  PANEL SSH BOT - PLANES SEPARADOS       ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}\n"
}
...
╔══════════════════════════════════════════════════════════════╗
║                                                              ║
║       🎉 INSTALACIÓN COMPLETADA - PLANES SEPARADOS 🎉      ║
║                                                              ║
...
echo -e "  ${CYAN}8.${NC} Recibe notificación cuando el pago sea aprobado\n"
...
echo -e "${GREEN}${BOLD}¡Sistema instalado exitosamente con planes separados! 🚀${NC}\n"
